...
%normalize.
fourier_signal = fft(Mix,Nr);
 % *%TODO* :
% Take the absolute value of FFT output
fourier_signal = abs(fourier_signal);
fourier_signal = fourier_signal ./ max(fourier_signal);
 % *%TODO* :
% Output of FFT is double sided signal, but we are interested in only one side of the spectrum.
% Hence we throw out half of the samples.

fourier_signal = fourier_signal(1 : Nr/2-1);
%plotting the range
...
%it a value of 1, else equate it to 0.4
RDM = RDM / max(RDM(:));
  ...
  for col1 = train_band + guard_band + 1 : (Nd) - (train_band + guard_band)
      ...
      for col2 = col1 - (train_band + guard_band) : col1 + (train_band + guard_band)
        if (abs(row1 - row2) > guard_cells || abs(col1 - col2) > guard_band)
          noise = noise + db2pow(RDM(row2, col2));
        end
      end
    end
   % Use RDM[x,y] as the matrix from the output of 2D FFT for implementing
   % CFAR
    threshold = pow2db(noise / (2 * (train_band + guard_band + 1) * 2 * (train_cells + guard_cells + 1) - (guard_cells * guard_band) - 1));
    threshold = threshold + offset;

    %cell_under_test = RDM(row1,col1);

    if (RDM(row1,col1) < threshold)
      RDM(row1, col1) = 0;
    else
      RDM(row1, col1) = 1;
    end
